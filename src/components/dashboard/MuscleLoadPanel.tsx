...
/** v2: el payload de fatiga guarda además las series y el último día entrenado. */
const HEATMAP_FATIGUE_CACHE_STORAGE_KEY = "gym-log.dashboard.heatmap-fatigue.v2";
...
/**
 * Mapa corporal con doble lectura, volumen de series o fatiga local.
 * Antes era el widget "Carga Muscular" del dashboard; ahora vive dentro del
 * detalle de fatiga muscular, así que se entrega como sección sin card propia.
 */
export function MuscleLoadPanel({
  onZoneSelect,
  className,
}: {
  onZoneSelect?: (group: MainMuscleGroup) => void;
  className?: string;
}) {
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      {/*
        La ventana de `useMuscleFatigue` es fija (28 días), así que los pills de
        periodo solo tienen sentido en modo volumen.
      */}
      {mode === "volume" && (
