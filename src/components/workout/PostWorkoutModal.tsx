...
import { Zap, Dumbbell, ArrowUp, CheckCircle2, Flame, Trophy, Timer } from "lucide-react";
...
export function PostWorkoutModal({
  ...
  const volumeLabel = breakdown.volumeLabel ?? "Volumen";
  const VolumeIcon = volumeLabel === "Duración" ? Timer : Dumbbell;
  ...
  const rows = [
    ...
    {
      key: "series",
      icon: <VolumeIcon className="h-4 w-4 text-primary" />,
      label: volumeLabel,
      value: seriesXp,
      className: "",
    },
